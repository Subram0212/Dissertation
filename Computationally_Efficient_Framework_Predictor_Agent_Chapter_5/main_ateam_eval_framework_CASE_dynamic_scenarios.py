...
import overall_PS_ARL_Scenario_dynamic_changes as PS
        ...
        for key, val in population_dict.items():
            ...
            for idx, value in enumerate(ordered_pts_from_depot):
                if idx == key[1]:
                    sp1 += 1
                    itm_stp_1 = value
                    itm_stp_1_dist += ugv_distance_calc_auto.distance_from_depotB_btw_ugvstops(strt_pt[0], itm_stp_1)
...
def local_minimum(X, num_uavs, ugv_stops_dict, improvement_process):
    ...
    minimum = minimize(fun=PS.persistent_surveillance_for_local_improver_agent, x0=X, args=(num_uavs, ugv_stops_dict, improvement_process, dynamic_added_mission_pt_pair[dynamic_mission_combo_num], current_loc), method='Nelder-Mead', callback=cb, options={'maxfev': 10}, tol=0.5)
    ...
    imp_fitness_val, imp_max_time, imp_depotb_vel, imp_ugv_vel, imp_track_node_age, imp_route_dict, imp_route_dict2, imp_route_dict3, imp_sp_feas_list, imp_ugv_list, imp_total_cost = PS.persistent_surveillance([int(imp_0), int(imp_1), int(imp_2), int(imp_3)], num_uavs, ugv_stops_dict, dynamic_added_mission_pt_pair[dynamic_mission_combo_num], current_loc)
...
def global_minimum(X_list, num_uavs, stops_comb_len, normalized_SA, clustering, label):
    ...
    # file_path = "Predictor agent classification plot/X_ensemble_val_10_"+str(ga_count)+".csv"
    ...
    # file_path_y = "Predictor agent classification plot/y_ensemble_10_"+str(ga_count)+".csv"
    # np.savetxt(file_path_y, predicted_cluster_list, delimiter=',', fmt='%d')

    # with open('output_10_ensemble_'+str(ga_count)+'.txt', 'w') as file:
...
reversed_ordered_pts_rest = list(reversed(ordered_pts_rest))
current_loc = [(0.6, 8.07)]
primary_rendz_locs = [(3.1, 7.23), (7.7, 3.61)]
ugv_stops_dict = {}
if current_loc[0] == primary_rendz_locs[0]:
    file_path = 'Replanning_1.yaml'
    with open(file_path, 'r') as file:
        yaml_data = yaml.load(file, Loader=yaml.FullLoader)
    route_dict = yaml_data['route_dict2']
    route_dict = ast.literal_eval(route_dict)
    visited_missions_so_far = [i[0] for i in route_dict]
...
# visited_missions_so_far = [(0.6, 8.07), (4.29, 11.21), (3.01, 8.87), (3.1, 7.24), (2.43, 7.91)]
# visited_missions_so_far = []
rs_num = 100
...
for n in range(len(sorted_initial_population)):
    ...
    fitness_val, max_time, depotb_vel, ugv_vel, track_node_age, route_dict, route_dict2, route_dict3, sp_feas_list, ugv_list, total_cost = PS.persistent_surveillance(param_list, num_uavs, ugv_stops_dict, dynamic_added_mission_pt_pair[dynamic_mission_combo_num], current_loc)
    temp = 0
    temp1 = 0
    temp2 = 0
    route_track_dict = {'sp1': [], 'sp2': [], 'sp3': []}
...
while flag == 1:
    ...
    print(len(x_copy))
    ensemble = clustering_algorithm_each_param_as_dist(overall_evaluated_solutions)
...
keys, vals = population_dict.items()
opt_sol = keys[0]
