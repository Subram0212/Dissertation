...
def create_data_model(sp3_end_point, stp1_ugv_stop_list, stp2_ugv_stop_list, se_act_depart_time, recharge_time, ugv_stops_dict, visited_missions, num_uavs, data_flag):
    n_depot_1 = 6
    n_stop_2 = 6
    data = {}
    ...
    df = pd.read_csv('Scenario 3 ordered rest.csv')
...
def print_solution(data, manager, routing, solution):  # Prints the UAV optimization solution output on the console.
    ...
    if data["end_loc"] != data["hyperparam_end_loc"]:
        df2_no_mid.to_excel('Sub_problem3_scenario3_1.xlsx', index=False, engine='openpyxl')
    else:
        df2_no_mid.to_excel('Sub_problem3_scenario3_2.xlsx', index=False, engine='openpyxl')
...
def main(sp3_end_point, stp1_ugv_stop_list, stp2_ugv_stop_list, se_act_depart_time, recharge_time, ugv_stops_dict, visited_missions, num_uavs, data_flag):  # This is where optimization of UAV routes happen.
    ...
    data, veh_max_time, depotb_vel, ugv_vel, tw_dict, ugv_stops_pairs, ugv_travel_time, ugv_inter_locs = create_data_model(sp3_end_point, stp1_ugv_stop_list, stp2_ugv_stop_list, se_act_depart_time, recharge_time, ugv_stops_dict, visited_missions, num_uavs, data_flag)
    # [END data]
    ...
    if solution:
        ...
        if obj_val < 1_000_000:
            ...
            return veh_max_time - total_time, veh_max_time, total_dist, total_time, depotb_vel, ugv_vel, route_dict, route, tw_dict, disttravel_track, ugv_travel_time, ugv_inter_locs
        else:
            # print("Objective value is: {}".format(obj_val))
            return obj_val, veh_max_time, total_dist, total_time, depotb_vel, ugv_vel, route_dict, route, tw_dict, disttravel_track, ugv_travel_time, ugv_inter_locs
    else:
        print("*************No solution found************")
        return 0, 0, 0, 0, ugv_vel, ugv_vel, [((0, 0), 0), ((0, 0), 0)], [0, 0], tw_dict, [0, 0], ugv_travel_time, ugv_inter_locs
    # [END print_solution]
