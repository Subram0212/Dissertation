...
def create_data_model(strt_pt, sp1_end_point, stp1_ugv_stop_list, ugv_stops_dict, visited_missions, num_uavs, data_flag, ordered_pts_rest, ordered_pts_from_depot):
    n_depot_1 = 6
    n_stop_1 = 6
    n_depot_2 = 6
    ugv_velocity = 15
    ...
    if data_flag == 1:
        df = pd.read_csv('Scenario 11 ordered from depot second.csv')
    else:
        df = pd.read_csv('Scenario 11 ordered from depot second.csv')
...
def print_solution(data, manager, routing, solution):  # Prints the UAV optimization solution output on the console.
    ...
    if data["end_loc"] != data["hyperparam_end_loc"]:
        df2_no_mid.to_excel('Sub_problem1_scenario3_1.xlsx', index=False, engine='openpyxl')
    else:
        df2_no_mid.to_excel('Sub_problem1_scenario3_2.xlsx', index=False, engine='openpyxl')
...
def main(strt_pt, sp1_end_point, stp1_ugv_stop, ugv_stops_dict, visited_missions, num_uavs, data_flag):  # This is where optimization of UAV routes happen.
    ...
    data, veh_max_time, depotb_vel, ugv_vel, tw_dict, ugv_inter_locs = create_data_model(strt_pt, sp1_end_point, stp1_ugv_stop, ugv_stops_dict, visited_missions, num_uavs, data_flag, ordered_pts_rest, ordered_pts_from_depot)
    # [END data]
