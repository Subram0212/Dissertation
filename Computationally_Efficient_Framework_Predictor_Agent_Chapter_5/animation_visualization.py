...
def add_circle_to_x(ax, x, y, drone_color, size=0.05, circle_radius=0.025):
...
# animation_data = pd.read_csv('Interpolated Fabricated route planning - Illustration data.csv')
# mission_locations_hardware = pd.read_csv('Fabricated data mission points.csv')
# mocap_data_x_uav1 = animation_data['x']
# mocap_data_y_uav1 = animation_data['y']
# mocap_data_x_uav2 = animation_data['x_uav2']
# mocap_data_y_uav2 = animation_data['y_uav2']
...
desired_length = len(mocap_data_x_uav1)
...
scn_map, = ax.plot(mission_locs_x, mission_locs_y, 'ko', markersize=5)
# Iterate over each frame and update the plot
for i in range(0, len(mocap_data_x_uav1)):
    ...
    patches_list1 = []
    # if i >= 25:
    #     scn_map_new, = ax.plot(2.45, 1.74, 'co', markersize=5)
    ...
    line1, line2, *circles = add_circle_to_x(ax, mocap_data_x_uav1[i], mocap_data_y_uav1[i], 'red')
    lines1.extend([line1, line2])
    patches_list1.extend(circles)
    plot_fig2 = ax.scatter(mocap_data_x_ugv[i], mocap_data_y_ugv[i], marker='s', s=100, color='b')
    ...
    if (mocap_data_x_uav1[i] in mission_locs_x_rnd or mocap_data_y_uav1[i] in mission_locs_y_rnd) or (mocap_data_x_uav1[i] == 2.45 or mocap_data_y_uav1[i] == 1.74):
        plot_tmp1, = ax.plot(mocap_data_x_uav1[i], mocap_data_y_uav1[i], 'ro', markersize=5, markeredgewidth=2)
    # if (mocap_data_x_uav2[i] in mission_locs_x_rnd or mocap_data_y_uav2[i] in mission_locs_y_rnd):
    #     plot_tmp1, = ax.plot(mocap_data_x_uav2[i], mocap_data_y_uav2[i], 'gx', markersize=5, markeredgewidth=2)
    if mocap_data_x_ugv[i] in mission_locs_x_rnd or mocap_data_y_ugv[i] in mission_locs_y_rnd:
        plot_tmp2, = ax.plot(mocap_data_x_ugv[i], mocap_data_y_ugv[i], 'bo', markersize=5, markeredgewidth=2)
