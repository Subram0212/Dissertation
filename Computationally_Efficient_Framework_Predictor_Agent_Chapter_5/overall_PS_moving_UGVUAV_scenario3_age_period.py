...
import sp1_scenario3_age_period as sp1
import sp2_scenario3_age_period as sp2
import sp3_scenario3_age_period as sp3
...
import scipy
import ugv_distance_calc_auto_scenario3 as ugv_distance_calc_auto
import math
...
mission_points = pd.read_csv('Scenario 7 data points.csv')
...
def perch_and_move_agents_age_period(location, ugv_direction, sp1_end_pt, sp2_end_pt, sp3_end_pt, ugv_velocity, data_flag, ordered_pts_rest):
    if data_flag == 1:
...
def persistent_surveillance_replanning(strt_pt, ugv_power, param_list, num_uavs, ugv_stops_dict, visited_missions_so_far, ugv_direction, stp_depart_time, recharge_time, subprob_cnt, visited_missions_so_far_with_time, added_mission_pts, Age_period):
    ...
    if count_1 or count_2 is None:
        ...
        data_flag = 1
    stp_travel_time = ugv_distance_calc_auto.distance_from_depotB(strt_pt_ft, ugv_opt_stop1_ft, ordered_pts_from_depot) // ugv_velocity
    ...
    while mission_time <= TOTAL_MISSION_TIME:
        ...
        while ugv_power <= ugv_max_capacity:
            ...
            if ugv_direction == 0:
                ...
                for i, node in enumerate(ordered_pts_rest):
                    if node != ugv_opt_stop1_tuple:
                        ...
                        for j in range(count_d_to_stp1, len(ordered_pts_rest)):
                            if ordered_pts_rest[j] != ugv_opt_stop2_tuple:
                ...
                timet = stp_depart_time
                for i in range(len(ugv_visits)-1):
                    each_stop_ugv_travel_T = ugv_distance_calc_auto.distance_btw_ugvstops(ugv_visits[i], ugv_visits[i+1], ordered_pts_rest) // ugv_velocity
                    timet += each_stop_ugv_travel_T
                    ugv_visits_time_track[ugv_visits[i+1]] = timet
                ugv_visits.pop(0)
                ...
                for i, node in enumerate(reversed_ordered_pts_rest):
                    if node != ugv_opt_stop2_tuple:
                        ...
                        for j in range(count_d_to_stp1, len(reversed_ordered_pts_rest)):
            ...
            while flag:
                ...
                if ugv_direction == 0:
                    veh_max_time = uav_strt_time + ugv_distance_calc_auto.distance_from_depotB_btw_ugvstops(fleeting_location[0], destination_loc[0], ordered_pts_from_depot) // ugv_velocity
                    fitness_val, mission_time, uav_time, total_distance, velocity1, velocity2, route_dict, route, tw_dict, disttravel_track, ugv_inter_locs = sp1.main(fleeting_location[0], ugv_opt_stop1, destination_loc, stp_depart_time, recharge_time, visited_missions, num_uavs, added_mission_pts, data_flag, Age_period)
                    if age_period_cnt == 0:
                        Age_period = {nodes: 0 for nodes in mission_points}
                        age_period_cnt += 1
                elif ugv_direction == 1:
                    veh_max_time = uav_strt_time + ugv_distance_calc_auto.distance_btw_ugvstops(fleeting_location[0], destination_loc[0], ordered_pts_rest) // ugv_velocity
                    fitness_val, mission_time, uav_time, total_distance, velocity1, velocity2, route_dict, route, tw_dict, disttravel_track, ugv_inter_locs = sp2.main(fleeting_location, ugv_opt_stop2, destination_loc, stp_depart_time, recharge_time, visited_missions, num_uavs, added_mission_pts, data_flag, ordered_pts_rest, Age_period)
                    if age_period_cnt == 0:
                        Age_period = {nodes: 0 for nodes in mission_points}
                        age_period_cnt += 1
                elif ugv_direction == 2:
                    veh_max_time = uav_strt_time + ugv_distance_calc_auto.distance_btw_ugvstops(fleeting_location[0], destination_loc[0], reversed_ordered_pts_rest) // ugv_velocity
                    fitness_val, mission_time, uav_time, total_distance, velocity1, velocity2, route_dict, route, tw_dict, disttravel_track, ugv_inter_locs = sp3.main(fleeting_location, ugv_opt_stop1, destination_loc, stp_depart_time, recharge_time, visited_missions, num_uavs, added_mission_pts, data_flag, Age_period)
                    if age_period_cnt == 0:
                        Age_period = {nodes: 0 for nodes in mission_points}
                        age_period_cnt += 1
                ...
                if route_dict[-2][0] == ugv_opt_stop1_tuple or route_dict[-2][0] == ugv_opt_stop2_tuple or route_dict[-2][0] == destination_loc or fitness_val == 0:
                    ...
                    recharge_time = time_on_UGV_charging
                ...
                for i in ugv_visits_dr_rhg:
                    visited_missions_so_far.append(i)
                for q, val in Age_period.items():
                    if len(route_locs_only) == 0:
                        break
                    if q not in route_locs_only:
                        if q in visited_missions_so_far:
                            Age_period[q] = Age_period[q] + (uav_time - uav_strt_time)
                        else:
                            Age_period[q] = uav_time
                    else:
                        idx_num = route_locs_only.index(q)
                        if q == ugv_strt_pt:
                            Age_period[q] = uav_time - uav_strt_time
                        elif q in ugv_visits:
                            if ugv_direction == 0:
                                if ugv_visits_time_track[q] > route_dict[idx_num][1] and ordered_pts_from_depot.index(q) < ordered_pts_from_depot.index(destination_loc[0]):
                                    Age_period[q] = uav_time - ugv_visits_time_track[q]
                                else:
                                    Age_period[q] = uav_time - route_dict[idx_num][1]
                            else:
                                Age_period[q] = uav_time - route_dict[idx_num][1]
                ...
                if destination_loc[0] == ugv_opt_stop1[0] or destination_loc[0] == ugv_opt_stop2[0]:
                    ...
                    if ugv_direction == 0:
                        ...
                        ugv_direction = 2
                        stp_travel_time = ugv_distance_calc_auto.distance(ugv_opt_stop1_ft, ugv_opt_stop2_ft, ordered_pts_rest) // ugv_velocity
                        stp_reach_time = stp_travel_time
                        if fitness_val == 0:
                            ugv_wait_time = 0
                        else:
                        ...
                        ugv_direction = 1
                        stp_travel_time = ugv_distance_calc_auto.distance(ugv_opt_stop1_ft, ugv_opt_stop2_ft, reversed_ordered_pts_rest) // ugv_velocity
                        stp_reach_time = stp_travel_time
                        if fitness_val == 0:
                            ugv_wait_time = 0
                        else:
                        ...
                        rend_cnt += 1
                    flag = 0
                fleeting_location, ugv_visits_dr_rhg = perch_and_move_agents_age_period(destination_loc, ugv_direction, ugv_opt_stop1, ugv_opt_stop2, ugv_opt_stop1, ugv_velocity, data_flag, ordered_pts_rest)
                if ugv_direction == 0:
                    cnt = 0
                    for i in range(cnt, len(ordered_pts_from_depot)):
                        if ordered_pts_from_depot[i] != fleeting_location[0]:
    ...
    while mission_time <= TOTAL_MISSION_TIME:
        ...
        while ugv_power <= ugv_max_capacity:
            ...
            if ugv_direction == 0:
                ...
                timet = stp_depart_time
                for i in range(len(ugv_visits)-1):
                    each_stop_ugv_travel_T = ugv_distance_calc_auto.distance_btw_ugvstops(ugv_visits[i], ugv_visits[i+1], reversed_ordered_pts_rest) // ugv_velocity
                    timet += each_stop_ugv_travel_T
                    ugv_visits_time_track[ugv_visits[i+1]] = timet
                ugv_visits.pop(0)
                ugv_strt_pt = ugv_visits[0]
    ...
    while ugv_power <= ugv_max_capacity:
        ...
        if ugv_max_capacity - ugv_power > ugv_power_check3 and stop2 is True and ugv_power_check3 > 0:
            ...
            for i in range(len(reversed_ugv_visits2)-1):
                each_stop_ugv_travel_T = ugv_distance_calc_auto.distance_btw_ugvstops(reversed_ugv_visits2[i], reversed_ugv_visits2[i+1], reversed_ordered_pts_rest) // ugv_velocity
                timet += each_stop_ugv_travel_T
                ugv_visits_time_track[reversed_ugv_visits2[i]] = timet
            ugv_visits_time_track[reversed_ugv_visits2[-1]] = timet + ugv_distance_calc_auto.distance_btw_ugvstops(reversed_ugv_visits2[-2], reversed_ugv_visits2[-1], reversed_ordered_pts_rest) // ugv_velocity
            stop1 = True
            stop2 = False
