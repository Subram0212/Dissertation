...
def create_data_model(stp1_ugv_stop_list, sp2_end_point, stp2_ugv_stop_list, nw_act_depart_time, recharge_time, visited_missions, num_uavs, added_mission_pts, data_flag, ordered_pts_rest, Age_period):
    n_depot_1 = 6
    n_stop_2 = 6
    data = {}
    data["vehicle_speed"] = 33
    ugv_velocity = 15
    nw_act_depart_time += recharge_time
    print(f'NW act depart time: {nw_act_depart_time}')
    fuel_capacity = 287700
    perch_move_agents_dict = {}
    for i in ordered_pts_rest:
        perch_move_agents_dict[i] = perch_and_move_agents(i, ugv_velocity)
    # df = pd.read_excel('ARL corridor reduced points ordered.xlsx')
    ...
    ordered_mission_pts_stp2_stop = []
    if data_flag == 1:
        df_list = list(reversed(df_list))
        for i in range(len(ordered_pts_rest)//2, len(ordered_pts_rest)):
            ordered_mission_pts_stp2_stop.append(ordered_pts_rest[i])
    else:
        for i in range(len(ordered_pts_rest)//2, len(ordered_pts_rest)):
            ordered_mission_pts_stp2_stop.append(ordered_pts_rest[i])
    # ordered_mission_pts_stp2_stop.extend(added_mission_pts)
    print(f"ordered mission points stp2: {ordered_mission_pts_stp2_stop}")
    ...
    ugv_stops_pairs = [stp1_ugv_stop_list[0]]
    time_btw_each_stop = distance_btw_each_stop // 13
    act_ugv_locs = [stp1_ugv_stop_list[0], ordered_pts_from_depot[0], ordered_pts_from_depot[0]]
    ...
    for i in range(len(ordered_mission_pts_stp2_stop)):
        count_se += 1
        if ordered_mission_pts_stp2_stop[i] == sp2_end_point[0]:
    ...
    ugv_new_travel_time = ugv_distance_calc_auto.distance_btw_ugvstops(stp1_ugv_stop_list[0], stp2_ugv_stop, ordered_pts_rest) // ugv_velocity
    se_stop_tw_1 = nw_act_depart_time + int(ugv_new_travel_time)
    se_stop_tw_2 = int(se_stop_tw_1 + 60)
    print(f'UGV velocity: {ugv_velocity}')
    if stp2_ugv_stop_list[0] == sp2_end_point[0]:
        veh_max_time = int(se_stop_tw_1 + 10000)
        uav_time_btw_stops = euclidean_distance(stp1_ugv_stop_ft, stp2_ugv_stop_ft) // data["vehicle_speed"]
        uav_depart_time = 0
        ugv_time_windows = ([(nw_act_depart_time, nw_act_depart_time+60)] +  # 0 (start)
                            [(0, veh_max_time)] * n_depot_1 +  # 1-6
                            [(se_stop_tw_1, veh_max_time)] * (n_stop_2-1) +  # 7-11
                            [(se_stop_tw_1, veh_max_time)] +  # 12
        ...
        veh_max_time = int(se_stop_tw_1 + 120)
        uav_time_btw_stops = euclidean_distance(stp1_ugv_stop_ft, stp2_ugv_stop_ft) // data["vehicle_speed"]
        # uav_depart_time = ugv_travel_time  # If you see just ugv_travel_time in this equation, Must change this! Subtract 'uav_time_btw_stops'
        uav_depart_time = 0
        ugv_time_windows = ([(nw_act_depart_time+60, nw_act_depart_time+120)] +  # 0 (start)
                            [(0, veh_max_time)] * n_depot_1 +  # 1-6
                            [(se_stop_tw_1, se_stop_tw_2)] * (n_stop_2-1) +  # 7-11
                            [(se_stop_tw_1, veh_max_time)] +  # 12
                            [(0, veh_max_time)] * total_mission_pts
                            )
        print("The UGV travel time between two UGV stops is: {}".format(ugv_travel_time))
    ...
    data["hyperparam_end_loc"] = sp2_end_point[0]
    data["num_vehicles"] = 1
    ...
    return data, veh_max_time, ugv_velocity, ugv_velocity, time_windows_dict, ugv_travel_time, locs_btw_stps
...
def print_solution(data, manager, routing, solution):  # Prints the UAV optimization solution output on the console.
    ...
    #     df2_no_mid.to_excel('Sub_problem2_hardware_implementation_2.xlsx', index=False, engine='openpyxl')
    # else:
    #     df2_no_mid.to_excel('Sub_problem2_hardware_implementation_1.xlsx', index=False, engine='openpyxl')
...
def main(stp1_ugv_stop_list, sp2_end_point, stp2_ugv_stop_list, nw_act_depart_time, recharge_time, visited_missions, num_uavs, added_mission_pts, data_flag, ordered_pts_rest, Age_period):  # This is where optimization of UAV routes happen.
    """Entry point of the program."""
    # Instantiate the data problem.
    # [START data]
    print(f'------------------------------------------------Subproblem 2-------------------------------------------')
    print(f'The corresponding UGV hyperparameters are: Stop2: {stp1_ugv_stop_list}')
    data, veh_max_time, depotb_vel, ugv_vel, tw_dict, ugv_travel_time, ugv_inter_locs = create_data_model(stp1_ugv_stop_list, sp2_end_point, stp2_ugv_stop_list, nw_act_depart_time, recharge_time, visited_missions, num_uavs, added_mission_pts, data_flag, ordered_pts_rest,  Age_period)
    # [END data]
    ...
    visits = data["visits"]
    MaxNoChange = 1.2  # 1000  # 1.2  # seconds
