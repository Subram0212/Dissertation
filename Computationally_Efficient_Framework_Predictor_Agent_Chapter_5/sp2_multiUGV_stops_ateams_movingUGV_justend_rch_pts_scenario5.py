...
def create_data_model(stp1_ugv_stop_list, sp2_end_point, stp2_ugv_stop_list, nw_act_depart_time, recharge_time, ugv_stops_dict, visited_missions, num_uavs, data_flag, ordered_pts_rest):
    n_depot_1 = 6
    n_stop_2 = 6
    data = {}
...
def print_solution(data, manager, routing, solution):  # Prints the UAV optimization solution output on the console.
    ...
    if data["end_loc"] != data["hyperparam_end_loc"]:
        df2_no_mid.to_excel('Sub_problem2_scenario3_1.xlsx', index=False, engine='openpyxl')
    else:
        df2_no_mid.to_excel('Sub_problem2_scenario3_2.xlsx', index=False, engine='openpyxl')
...
def main(stp1_ugv_stop_list, sp2_end_point, stp2_ugv_stop_list, nw_act_depart_time, recharge_time, ugv_stops_dict, visited_missions, num_uavs, data_flag):  # This is where optimization of UAV routes happen.
    """Entry point of the program."""
    # Instantiate the data problem.
    # [START data]
    print(f'------------------------------------------------Subproblem 2-------------------------------------------')
    print(f'The corresponding UGV hyperparameters are: Stop2: {stp1_ugv_stop_list}')
    data, veh_max_time, depotb_vel, ugv_vel, tw_dict, ugv_travel_time, ugv_inter_locs = create_data_model(stp1_ugv_stop_list, sp2_end_point, stp2_ugv_stop_list, nw_act_depart_time, recharge_time, ugv_stops_dict, visited_missions, num_uavs, data_flag, ordered_pts_rest)
    # [END data]
