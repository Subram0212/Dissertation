...
# def clustering_algorithm(X, y, population_dict):
#     cnt = 0
...
#     X_arr = np.array(X)
#     y_arr = np.array(y)
#     kmeans.fit(X_arr, y_arr)
#     cluster_labels = kmeans.predict(X_arr)
#     cluster_dict = {}
#
#     for i, label in enumerate(cluster_labels):
#         if label not in cluster_dict:
#             cluster_dict[label] = X_arr[i]
#
#     dpgmm = BayesianGaussianMixture(n_components=5, max_iter=10000000, tol=1e-5, init_params='kmeans',
#                                     weight_concentration_prior_type='dirichlet_process', weight_concentration_prior=1, random_state=0)
#     dpgmm.fit(X_arr, y_arr)
#     n_to_select = 3
#     w = (-dpgmm.weights_).argsort()[:n_to_select]
#     mat = []
#     eps = 1e-16
#     for ind2 in w:
#         p1 = sp.multivariate_normal.pdf(X_arr, mean=dpgmm.means_[ind2], cov=dpgmm.covariances_[ind2]) + eps
#         mat.append(p1)
...
def clustering_algorithm_each_param_as_dist(population_dict):
    ...
    # y_class_arr = y_class_arr.reshape(-1, 1)
    # data = np.hstack((X_arr, y_arr))
    # kmeans.fit(X_arr)
    # cluster_labels = kmeans.predict(X_arr)
    # cluster_dict = {0: [], 1: [], 2: []}
    #
    # for i, label in enumerate(cluster_labels):
    #     # if label not in cluster_dict:
    #     y_vals = y_arr[i].tolist()
    #     cluster_dict[label].append(y_vals[0])

    knn = KNeighborsClassifier()
    y_arr = y_arr.ravel()
    knn.fit(X_arr, y_arr)

    # dpgmm = BayesianGaussianMixture(n_components=3, max_iter=10000000, tol=1e-5, init_params='kmeans',
    #                                 weight_concentration_prior_type='dirichlet_process', weight_concentration_prior=1, random_state=0)
    # data = np.hstack((X_arr, y_arr))
    # dpgmm.fit(X_arr)
    # n_to_select = 3
...
def local_minimum(X, num_uavs, ugv_stops_dict, improvement_process):
    ...
    imp_fitness_val, imp_max_time, imp_depotb_vel, imp_ugv_vel, imp_track_node_age, imp_route_dict, imp_route_dict2, imp_route_dict3, imp_sp_feas_list, imp_ugv_list, imp_total_cost = PS.persistent_surveillance([int(imp_0), int(imp_1), int(imp_2), int(imp_3)], num_uavs, ugv_stops_dict)
...
def global_minimum(X_list, num_uavs, stops_comb_len, normalized_SA, clustering, label):
    ...
    # file_path = "X_knn_val_50_"+str(ga_count)+".csv"
    ...
    # file_path_y = "y_knn_val_50_"+str(ga_count)+".csv"
    # np.savetxt(file_path_y, predicted_cluster_list, delimiter=',', fmt='%d')

    # with open('output_kNN_50_'+str(ga_count)+'.txt', 'w') as file:
...
# def wrapper3(func, arg1, arg2, arg3, arg4, queue):
#     queue.put(func(arg1, arg2, arg3, arg4))


# For first iteration (comment it out if replanning is performed)
# for key, val in population_dict.items():
#     if val > 5_000_000:  # This statement checks if all the solutions in the population are feasible
#         flag = 1
#         break
while flag == 1:
    ga_count += 1
    ...
    thread2 = Thread(target=wrapper2, args=(global_minimum, evolution_with_obj_val, num_uavs, stops_comb_len, normalized_SA, knn, infeasible_label, q2)).start()
