...
import overall_PS_moving_UGVUAV_scenario3_age_period as PS
import overall_PS_multiUGVstops_UGVUAVmoving_scenario3 as PS2
        ...
        for key, val in population_dict.items():
            ...
            for idx, value in enumerate(ordered_pts_from_depot):
                if idx == key[1]:
                    sp1 += 1
                    itm_stp_1 = value
                    itm_stp_1_dist += ugv_distance_calc_auto.distance_from_depotB_btw_ugvstops(mission_strt_pt[0], itm_stp_1, ordered_pts_from_depot)
...
mission_points_df = pd.read_csv('Scenario 7 data points.csv')
...
ugv_stops_dict = {}
ugv_Stops = minimum_set_cover_UGV_hyperparam_refined_replanning.main(ordered_pts_rest_df, 'Case_Study_scenario7.csv', rs_num, visited_missions_so_far, current_loc)
