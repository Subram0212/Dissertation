...
import overall_PS_multiUGVstops_UGVUAVmoving_scenario3 as PS
...
def global_minimum(X_list, num_uavs, stops_comb_len, normalized_SA, clustering, label):
    ...
    file_path = "Predictor agent classification plot scenario 3/X_ensemble_val_500_scn3_"+str(ga_count)+".csv"  # Change this name while varying the classifier
    ...
    file_path_y = "Predictor agent classification plot scenario 3/y_ensemble_val_500_scn3_"+str(ga_count)+".csv"   # Change this name while varying the classifier
    np.savetxt(file_path_y, predicted_cluster_list, delimiter=',', fmt='%d')

    with open('output_500_scn3_ensemble_'+str(ga_count)+'.txt', 'w') as file:
...
ordered_pts_rest_df = pd.read_csv('Scenario 7 ordered rest.csv')
...
reversed_ordered_pts_rest = list(reversed(ordered_pts_rest))
ugv_stops_dict = {}
ugv_Stops = minimum_set_cover_UGV_hyperparam_refined.main(ordered_pts_rest_df, 'Case_Study_scenario7.csv')
