...
# ordered_pts_rest = pd.read_csv('Hardware experimental scaled up scenario in miles ordered.csv')
ordered_pts_from_depot = pd.read_csv('Scenario 7 ordered from depot.csv')
ordered_pts_rest = pd.read_csv('Scenario 7 ordered rest.csv')
ordered_pts_from_depot_second = pd.read_csv('Scenario 7 ordered from depot second.csv')
...
def create_data_model(stp1_ugv_stop_list, sp3_end_point, stp2_ugv_stop_list, se_act_depart_time, recharge_time, visited_missions, num_uavs, added_mission_pts, data_flag, Age_period):
    n_depot_1 = 6
    n_stop_2 = 6
    data = {}
    fuel_capacity = 287700
    se_act_depart_time += recharge_time
    # branch_mid_pt = [(3.79, 6.71)]
    # branch_mid_pt = [(3.72, 6.86)]
    branch_mid_pt = [(1.1, 3.85)]
    print(f'SE act depart time: {se_act_depart_time}')
    data["vehicle_speed"] = 33
    ugv_velocity = 15
    perch_move_agents_dict = {}
    for i in ordered_pts_rest:
        perch_move_agents_dict[i] = perch_and_move_agents(i, ugv_velocity)
    df = pd.read_csv('Scenario 7 ordered rest.csv')
    ...
    df_list = list(reversed(df_list))

    ordered_mission_pts_stp1_stop = []
    for j in ordered_pts_from_depot:
        if j != branch_mid_pt[0]:
            ordered_mission_pts_stp1_stop.append(j)
        else:
            break
    original_ordered_pts_rest = list(reversed(ordered_pts_rest))
    if data_flag == 1:
        df_list = list(reversed(df_list))
        for i in range(len(ordered_pts_rest)//2):
            ordered_mission_pts_stp1_stop.append(ordered_pts_rest[i])
    else:
        for i in range(len(original_ordered_pts_rest)//2):
            df_list = list(reversed(df_list))
            ordered_mission_pts_stp1_stop.append(original_ordered_pts_rest[i])
    # ordered_mission_pts_stp1_stop.extend(added_mission_pts)
    print(f"Added mission points: {added_mission_pts}")
    print(f"ordered mission pts: {ordered_mission_pts_stp1_stop}")
    # ordered_mission_pts_stp1_stop.append((6.5, 7.5))
    # ordered_mission_pts_stp1_stop.append((3.75, 4.75))

    # ordered_mission_pts_stp1_stop = [(4.29, 11.21), (4.01, 10.83), (3.74, 10.45),
    #                                  (3.54, 10.04), (0.63, 7.97), (0.75, 7.58), (1.2, 6.82), (1.54, 6.48), (2.44, 6.46), (3.37, 6.33),
    #                                  (3.42, 9.61), (3.29, 9.23), (3.01, 8.87), (2.61, 8.34), (2.43, 7.91), (2.77, 7.57), (3.1, 7.24),
    ...
    dist = ugv_distance_calc_auto.distance(stp1_ugv_stop_ft, stp2_ugv_stop_ft, ordered_pts_rest)
    ugv_travel_time = (dist // ugv_velocity)
    _locations = []
    locs_btw_stps = []
    count = 0
    count_se = 0
    for j in range(len(df_list)):
        if df_list[j] != stp1_ugv_stop and count_se == 0:
            continue
        count_se += 1
        count += 1
        if df_list[j] == stp2_ugv_stop:
            for k in range(df_list.index(stp1_ugv_stop), df_list.index(stp1_ugv_stop)+count):
                locs_btw_stps.append(df_list[k])

    distance_btw_each_stop = 0
    ugv_stops_pairs = [stp2_ugv_stop_list[0]]
    time_btw_each_stop = distance_btw_each_stop // ugv_velocity
    act_ugv_locs = [stp2_ugv_stop_list[0], ordered_pts_from_depot[0], ordered_pts_from_depot[0]]
    _locations = (stp1_ugv_stop_list +
                  [ordered_pts_from_depot[0]] * n_depot_1 +
                  stp2_ugv_stop_list * (n_stop_2 - 1) +
                  stp2_ugv_stop_list
                  )
    n_sum_rch_stops = n_depot_1 + n_stop_2
    print(locs_btw_stps)
    mission_locations = []
    count = 0
    count_se = 0
    for i in range(len(ordered_mission_pts_stp1_stop)):
        count += 1
        if ordered_mission_pts_stp1_stop[i] == sp3_end_point[0]:
            for j in range(count-1):
                mission_locations.append(ordered_mission_pts_stp1_stop[j])
            break
        else:
            continue
    print(f"visited missions: {visited_missions}")
    print(f"Mission_locations: {mission_locations}")
    mission_locations.extend(added_mission_pts)
    for o in visited_missions:
        if o in mission_locations:
            mission_locations.remove(o)
    total_mission_pts = len(mission_locations)
    print(f"Mission locations: {mission_locations}")
    ...
    ugv_new_travel_time = ugv_distance_calc_auto.distance_btw_ugvstops(stp2_ugv_stop_list[0], stp1_ugv_stop, ordered_pts_rest) // ugv_velocity
    nw_stop_tw_1 = int(se_act_depart_time + ugv_new_travel_time)
    nw_stop_tw_2 = int(nw_stop_tw_1 + 60)
    print(f'UGV velocity: {ugv_velocity}')
    print("The UGV travel time between two UGV stops is: {}".format(ugv_travel_time))
    if stp2_ugv_stop_list[0] == sp3_end_point[0]:
        veh_max_time = int(nw_stop_tw_1 + 10000)
        uav_time_btw_stops = euclidean_distance(stp1_ugv_stop_ft, stp2_ugv_stop_ft) // data["vehicle_speed"]
        uav_depart_time = 0  # If you see just ugv_travel_time in this equation, Must change this! Subtract 'uav_time_btw_stops'
        ugv_time_windows = ([(se_act_depart_time, se_act_depart_time+60)] +  # 0 (start)
                            [(0, veh_max_time)] * n_depot_1 +  # 1-6
                            [(nw_stop_tw_1, veh_max_time)] * (n_stop_2-1) +  # 7-11
                            [(nw_stop_tw_1, veh_max_time)] +  # 12
        ...
        ugv_time_windows = ([(se_act_depart_time, se_act_depart_time+60)] +  # 0 (start)
                            [(0, veh_max_time)] * n_depot_1 +  # 1-6
                            [(nw_stop_tw_1, nw_stop_tw_2)] * (n_stop_2-1) +  # 7-11
                            [(nw_stop_tw_1, veh_max_time)] +  # 12
                            [(0, veh_max_time)] * total_mission_pts
                            )
        print(ugv_time_windows)
    ...
    data['counter'] = ([0] +  # 0 (start)
                       [0] * (n_sum_rch_stops - 1) +  # 1-11 (stations)
                       [0] +  # 12 (end)
                       [1] * total_mission_pts)
    data["start_loc"] = stp1_ugv_stop
    data["end_loc"] = stp2_ugv_stop
    data["hyperparam_end_loc"] = sp3_end_point[0]
    data["num_vehicles"] = 1
    ...
    data['Priotized penalty'] = [int((((Age_period[x]))/60)**1) for x in data['coordinates']]
    data['Priotized penalty dict'] = {x: int((((Age_period[x]))/60)**1) for x in data['coordinates']}
    print(data['Priotized penalty dict'])
    ...
    return data, veh_max_time, ugv_velocity, ugv_velocity, time_windows_dict, ugv_stops_pairs, ugv_travel_time, locs_btw_stps
...
def print_solution(data, manager, routing, solution):  # Prints the UAV optimization solution output on the console.
    ...
    # fuel_var = fuel_dimension.CumulVar(routing.End(vehicle_id))
    # time_elapsed.append(round(total_time/60))
    # fuel_remaining.append(solution.Value(fuel_var))
    ...
    #     df2_no_mid.to_excel('Sub_problem3_hardware_implementation_2.xlsx', index=False, engine='openpyxl')
    # else:
    #     df2_no_mid.to_excel('Sub_problem3_hardware_implementation_1.xlsx', index=False, engine='openpyxl')
    ...
    for route_nbr in range(routing.vehicles()):
        ...
        while not routing.IsEnd(index):
            ...
            if type(data["coordinates"][node][0]) == float:
                ...
                dist_btw_nodes = ugv_distance_calc_auto.distance_btw_ugvstops(data["coordinates"][node][0], data["coordinates"][node][1], ordered_pts_rest)
            ...
            # else:
            #     dist_btw_nodes = ugv_distance_calc_auto.distance_btw_ugvstops(data["coordinates"][node][0], data["coordinates"][node][1])
            #     time_btw_nodes = int(dist_btw_nodes / ugv_speed)
...
class SolutionCallback(object):
    ...
    def __call__(self):
        global time_limit, time_limit_counter
        self.solution_count += 1
        CurrObj = self.model.CostVar().Min()
        CurrTime = time()
...
def main(sp3_end_point, stp1_ugv_stop_list, stp2_ugv_stop_list, se_act_depart_time, recharge_time, visited_missions, num_uavs, added_mission_pts, data_flag, Age_period):  # This is where optimization of UAV routes happen.
    """Entry point of the program."""
    # Instantiate the data problem.
    # [START data]
    print(f'------------------------------------------------Subproblem 3-------------------------------------------')
    print(f'The corresponding UGV hyperparameters are: Stop1: {stp1_ugv_stop_list}, Stop2: {stp2_ugv_stop_list}')
    data, veh_max_time, depotb_vel, ugv_vel, tw_dict, ugv_stops_pairs, ugv_travel_time, ugv_inter_locs = create_data_model(sp3_end_point, stp1_ugv_stop_list, stp2_ugv_stop_list, se_act_depart_time, recharge_time, visited_missions, num_uavs, added_mission_pts, data_flag, Age_period)
    # [END data]
    ...
    visits = data["visits"]
    MaxNoChange = 1.3  # 1000  # 1.2  # seconds
    ...
    for node in range(len(data["distance_matrix"])):
        ...
        if node in stations:
            ...
            visit_penalty = int(data['Priotized penalty'][node] * 10000)
    ...
    if solution:
        ...
        if obj_val < 1_000_000:
            ...
            return veh_max_time - total_time, veh_max_time, total_time, total_dist, depotb_vel, ugv_vel, route_dict, route, tw_dict, disttravel_track, ugv_inter_locs
        else:
            # print("Objective value is: {}".format(obj_val))
            return obj_val, veh_max_time, total_time, total_dist, depotb_vel, ugv_vel, route_dict, route, tw_dict, disttravel_track, ugv_inter_locs
    else:
        print("*************No solution found************")
        return 0, 0, 0, 0, ugv_vel, ugv_vel, [((0, 0), 0), ((0, 0), 0)], [0, 0], tw_dict, [0, 0], ugv_inter_locs
    # [END print_solution]
