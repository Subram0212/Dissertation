...
def modified_CP(df, visited_missions_so_far, current_loc):
    def cumulative_distance_determination (df, visited_missions_so_far):
        df = pd.read_csv(df)
        X = df['X']
        Y = df['Y']
        branch1 = []
        ...
        # branch1 = [i for i in branch1 if i not in visited_missions_so_far]
        # branch2 = [i for i in branch2 if i not in visited_missions_so_far]
        # branch3 = [i for i in branch3 if i not in visited_missions_so_far]
    ...
    cu_dis,A = cumulative_distance_determination(df, visited_missions_so_far)
    ...
    class VarArraySolutionPrinter(cp_model.CpSolverSolutionCallback):
        ...
        def on_solution_callback(self):
            ...
            hit_rendezvous = hit(current_loc[0],Targets,Fuel_limit)
            remv=[]
            for i in hit_start:
                remv.append(Targets[i])
            for i in hit_rendezvous:
                if Targets[i] not in remv:
                    remv.append(Targets[i])
            for i in remv:
                Targets.remove(i)
            if starting[0] in A:
            ...
            #  print('\n')
            self.Sol.append(current_loc+Rnd_p)
    ...
    def CP_set_cover(df):
        ...
        hit_start= hit(starting[0],Targets,Fuel_limit)
        hit_rendezvous = hit(current_loc[0],Targets,Fuel_limit)
        hit_point = hit(branch3[5], Targets, Fuel_limit)
        remv=[]
        for i in hit_start:
            remv.append(Targets[i])
        for i in hit_rendezvous:
        ...
        for i in range(len(A)):
            if A[i] in visited_missions_so_far and A[i] != (X[7], Y[7]):
                continue
            i_list.append(str(i))
            ...
            obj_var.setdefault('Point {}'.format(i),(cu_dis1(A[i],current_loc[0])))
        ...
        for a, b in hit_set.items():
            for k in b:
                if k not in i_list:
                    continue
                idx = list(point_vars.keys()).index('Point '+str(k))
                target_set['{}'.format(a)].add(list(point_vars.values())[idx])
        ...
        for i in range(len(Targets)):
            ...
            hit_set.setdefault('Target {}'.format(i), set())
        for i in range(len(A)):
            if A[i] in visited_missions_so_far:
                continue
            p_var= point_vars.setdefault('Point {}'.format(i), model.NewBoolVar('Point '+str(i)))
            hitted_dict.setdefault('{}'.format(i), set(hit(A[i],Targets,Fuel_limit)))
            obj_var.setdefault('Point {}'.format(i),(cu_dis1(A[i],current_loc[0])))
...
def main(scenario_points, df, rs_num, visited_missions_so_far, current_loc):
    ...
    solutions, endpt1, endpt2, Startpt = modified_CP(df, visited_missions_so_far, current_loc)
    # print(solutions)
    start_pt = []
    ugv_stops = {}
