...
import ugv_distance_calc_auto_scenario2
...
with open('output_100_scn2_kNN.txt', 'r') as file:
    for line in file:
        values = line.split()
        if len(values) >= 5:
...
plt.savefig('Confusion matrix and scores/confusion_matrix - kNN, scenario 2 and seed 100.pdf', format='pdf')
...
plt.savefig('Confusion matrix and scores/ROC curve - kNN, scneario 2 and seed 100.pdf', format='pdf')
