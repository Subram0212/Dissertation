...
import sp1_multiUGVstops_ateams_movingUGV_justend_rch_pts_scenario4 as sp1
# import sp1_multiUGVstops_ateams_movingUGV as sp1
import sp2_multiUGV_stops_ateams_movingUGV_justend_rch_pts_scenario4 as sp2
# import sp2_multiUGV_stops_ateams_movingUGV as sp2
import sp3_multiUGV_stops_ateams_movingUGV_justend_rch_pts_scenario4 as sp3
...
import scipy
import ugv_distance_calc_auto_scenario4 as ugv_distance_calc_auto


mission_points = pd.read_csv('Scenario 3 data points.csv')
ordered_pts_from_depot = pd.read_csv('Scenario 3 ordered from depot.csv')
ordered_pts_rest = pd.read_csv('Scenario 3 ordered rest.csv')
ordered_pts_from_depot_second = pd.read_csv('Scenario 3 ordered from depot second.csv')
...
def perch_and_move_agents(location, sp1, sp2, sp3, sp1_end_pt, sp2_end_pt, sp3_end_pt, ugv_velocity, data_flag, ordered_pts_rest):
...
def persistent_surveillance(param_list, num_uavs, ugv_stops_dict):
    ...
    while flag:
        ...
        if len(visited_missions) == len(ordered_pts_from_depot) - depot_to_stp_len:
            break
        fleeting_location = perch_and_move_agents(destination_loc, sp1_flag, sp2_flag, sp3_flag, ugv_opt_stop1, ugv_opt_stop2, ugv_opt_stop1, ugv_velocity, data_flag, ordered_pts_rest)
        destination_loc = ugv_opt_stop1
        for m in route_dict:
    ...
    while ugv_power <= ugv_max_capacity:
        ...
        if (ugv_power <= ugv_power_check1 or ugv_power <= ugv_power_check2) and stop1 is True and ugv_power_check2 > 0:  # The condition says, if the ugv power consumption is less than the power that is remaining after it has consumed by traveling to respective locations, then continue in the condition.
            subprob_cnt += 1
            sp2_flag = 1
            fleeting_location = perch_and_move_agents(ugv_opt_stop1, sp1_flag, sp2_flag, sp3_flag, ugv_opt_stop1, ugv_opt_stop2, ugv_opt_stop1, ugv_velocity, data_flag, ordered_pts_rest)
            end_pt_1_id = 0
        ...
        if (ugv_max_capacity - ugv_power > ugv_power_check3 or ugv_max_capacity - ugv_power > ugv_power_btw_stops) and stop2 is True and ugv_power_check3 > 0:
            subprob_cnt += 1
            sp3_flag = 1
            fleeting_location = perch_and_move_agents(ugv_opt_stop2, sp1_flag, sp2_flag, sp3_flag, ugv_opt_stop1, ugv_opt_stop2, ugv_opt_stop1, ugv_velocity, data_flag, ordered_pts_rest)
...
if __name__ == "__main__":
    ...
    param_list = [41, 4, 4, 6]  # optimal UGV solution for the reduced scenario - RS = 100
    ...
    num_uavs = 1
    ugv_stops_dict = {}
    mission_points_ordered_df = pd.read_csv('Scenario 3 ordered rest.csv')
    ...
    ugv_Stops = minimum_set_cover_UGV_hyperparam_refined.main(mission_points_ordered_df, 'Case_Study_scenario3.csv')
