...
def create_data_model(strt_pt, sp1_end_point, stp1_ugv_stop_list, stp_depart_time, recharge_time, visited_missions, num_uavs, added_mission_pts, data_flag, Age_period):
    global ordered_pts_from_depot
    global ordered_pts_rest
    n_depot_1 = 6
    n_stop_1 = 6
    n_depot_2 = 6
    ugv_velocity = 15
    ...
    if data_flag == 1:
        ...
        df = pd.read_csv('Scenario 7 ordered from depot second.csv')
    else:
        df = pd.read_csv('Scenario 7 ordered from depot.csv')
    ...
    uav_strt_time = stp_depart_time + recharge_time
    nw_stop_tw_1 = (ugv_distance_calc_auto.distance_from_depotB_btw_ugvstops(strt_pt, stp1_ugv_stop_list[0], ordered_pts_from_depot) // ugv_velocity) + uav_strt_time
...
def print_solution(data, manager, routing, solution):  # Prints the UAV optimization solution output on the console.
    ...
    #     df2_no_mid.to_excel('Sub_problem1_hardware_implementation_2.xlsx', index=False, engine='openpyxl')
    # else:
    #     df2_no_mid.to_excel('Sub_problem1_hardware_implementation_1.xlsx', index=False, engine='openpyxl')
...
def main(strt_pt, sp1_end_point, stp1_ugv_stop, stp_depart_time, recharge_time, visited_missions, num_uavs, added_mission_pts, data_flag, Age_period):  # This is where optimization of UAV routes happen.
    """Entry point of the program."""
    # Instantiate the data problem.
    # [START data]
    print(f'------------------------------------------------Subproblem 1-------------------------------------------')
    print(f'The corresponding UGV hyperparameters are: Stop1: {stp1_ugv_stop}')
    data, veh_max_time, depotb_vel, ugv_vel, tw_dict, ugv_inter_locs = create_data_model(strt_pt, sp1_end_point, stp1_ugv_stop, stp_depart_time, recharge_time, visited_missions, num_uavs, added_mission_pts, data_flag, Age_period)
    # [END data]
