import pandas as pd
import numpy as np
import minimum_set_cover_UGV_hyperparam_refined_scenario5 as minimum_set_cover_UGV_hyperparam_refined
...
import sp1_multiUGVstops_ateams_movingUGV_justend_rch_pts_scenario5 as sp1
# import sp1_multiUGVstops_ateams_movingUGV as sp1
import sp2_multiUGV_stops_ateams_movingUGV_justend_rch_pts_scenario5 as sp2
# import sp2_multiUGV_stops_ateams_movingUGV as sp2
import sp3_multiUGV_stops_ateams_movingUGV_justend_rch_pts_scenario5 as sp3
...
mission_points = pd.read_csv('Scenario 11 data points.csv')
ordered_pts_from_depot = pd.read_csv('Scenario 11 ordered from depot.csv')
ordered_pts_rest = pd.read_csv('Scenario 11 ordered rest.csv')
ordered_pts_from_depot_second = pd.read_csv('Scenario 11 ordered from depot second.csv')
...
def perch_and_move_agents(location, sp1, sp2, sp3, sp1_end_pt, sp2_end_pt, sp3_end_pt, ugv_velocity, data_flag, ordered_pts_rest, ordered_pts_from_depot):
    recharge_constant_time = 926
    uav_act_depart_location = []
    cumul_dist = 0
    cnt = 0
    reversed_ordered_pts_rest = list(reversed(ordered_pts_rest))
    print(f"data flag in the distance calc: {data_flag}")
    if data_flag == 1:
            ...
            if i != location[0]:
                ...
                for j in range(cnt, len(reversed_ordered_pts_rest)):
                    if j == len(reversed_ordered_pts_rest)-1:
                        break
                    temp = ugv_distance_calc_auto.distance_btw_ugvstops(reversed_ordered_pts_rest[j], reversed_ordered_pts_rest[j+1], reversed_ordered_pts_rest) // ugv_velocity
                    cumul_dist += temp
                    if cumul_dist >= recharge_constant_time:
...
def persistent_surveillance(param_list, num_uavs, ugv_stops_dict):
    ...
    for i, list_val in enumerate(ordered_pts_from_depot):
        if ordered_pts_from_depot[i] == (round(strt_pt[0][0], 2), round(strt_pt[0][1], 2)):
            count_1 = i
        if ordered_pts_from_depot[i] == (round(ugv_opt_stop1[0][0], 2), round(ugv_opt_stop1[0][1], 2)):
            count_2 = i
    print(count_1, count_2)
    print(ordered_pts_from_depot)
    print(strt_pt, ugv_opt_stop1)
    if count_1 is None or count_2 is None:
        ordered_pts_from_depot = ordered_pts_from_depot_second
        ordered_pts_rest = list(reversed(ordered_pts_rest))
        data_flag = 1
    stp1_travel_time = ugv_distance_calc_auto.distance_from_depotB(strt_pt_ft, ugv_opt_stop1_ft, ordered_pts_from_depot) // ugv_velocity
    travel_time_btw_stops = ugv_distance_calc_auto.distance(ugv_opt_stop1_ft, ugv_opt_stop2_ft, ordered_pts_rest) // ugv_velocity
    ...
    visited_missions = []
    if uav_rendezvous_arrvl_location_sp1 >= len(ordered_pts_from_depot):
        uav_rendezvous_arrvl_location_sp1 = len(ordered_pts_from_depot)-1
    ...
    while flag:
        ...
        fitness_val, mission_time, total_distance, uav_time, velocity1, velocity2, route_dict, route, tw_dict, disttravel_track, ugv_inter_locs = sp1.main(fleeting_location[0], ugv_opt_stop1, destination_loc, ugv_stops_dict, visited_missions, num_uavs, data_flag)
        ...
        if len(visited_missions) == len(ordered_pts_from_depot) - depot_to_stp_len:
            break
        print(f"data flag: {data_flag}")
        print(f"ordered_pts_from_depot: {ordered_pts_from_depot}")
        fleeting_location = perch_and_move_agents(destination_loc, sp1_flag, sp2_flag, sp3_flag, ugv_opt_stop1, ugv_opt_stop2, ugv_opt_stop1, ugv_velocity, data_flag, ordered_pts_rest, ordered_pts_from_depot)
        destination_loc = ugv_opt_stop1
        for m in route_dict:
    ...
    for i in range(len(ugv_visits1)-1):
        each_stop_ugv_travel_T = ugv_distance_calc_auto.distance_from_depotB_btw_ugvstops(ugv_visits1[i], ugv_visits1[i+1], ordered_pts_from_depot) // ugv_velocity
    ...
    while ugv_power <= ugv_max_capacity:
        ...
        if (ugv_power <= ugv_power_check1 or ugv_power <= ugv_power_check2) and stop1 is True and ugv_power_check2 > 0:  # The condition says, if the ugv power consumption is less than the power that is remaining after it has consumed by traveling to respective locations, then continue in the condition.
            subprob_cnt += 1
            sp2_flag = 1
            fleeting_location = perch_and_move_agents(ugv_opt_stop1, sp1_flag, sp2_flag, sp3_flag, ugv_opt_stop1, ugv_opt_stop2, ugv_opt_stop1, ugv_velocity, data_flag, ordered_pts_rest, ordered_pts_from_depot)
            end_pt_1_id = 0
            ...
            while flag2:
                ...
                fitness_val2, mission_time2, total_distance2, uav_time, velocity1_2, velocity2_2, route_dict2, route2, tw_dict2, disttravel_track2, ugv_travel_time, ugv_inter_locs = sp2.main(fleeting_location, ugv_opt_stop2, destination_loc, stp1_depart_time, recharge_time, ugv_stops_dict, visited_missions, num_uavs, data_flag)
            ...
            count = 0
            print(ordered_pts_rest)
            for i, node in enumerate(ordered_pts_rest):
                count += 1
                if node == ugv_opt_stop1_tuple:
                    for j in range(count, len(ordered_pts_rest)):
            ...
            for i in range(len(ugv_visits2)-1):
                each_stop_ugv_travel_T = ugv_distance_calc_auto.distance_btw_ugvstops(ugv_visits2[i], ugv_visits2[i+1], ordered_pts_rest) // ugv_velocity
                timet += each_stop_ugv_travel_T
                ugv_visits_time_track[ugv_visits2[i]] = timet
            if len(ugv_visits2) >= 2:
                ugv_visits_time_track[ugv_visits2[-1]] = timet + ugv_distance_calc_auto.distance_btw_ugvstops(ugv_visits2[-2], ugv_visits2[-1], ordered_pts_rest) // ugv_velocity
            else:
                ugv_visits_time_track[ugv_visits2[-1]] = timet
            stop2 = True
            stop1 = False
            ...
            visited_missions = []
            ugv_travel_time_tot = 0
        if (ugv_max_capacity - ugv_power > ugv_power_check3 or ugv_max_capacity - ugv_power > ugv_power_btw_stops) and stop2 is True and ugv_power_check3 > 0:
            subprob_cnt += 1
            sp3_flag = 1
            fleeting_location = perch_and_move_agents(ugv_opt_stop2, sp1_flag, sp2_flag, sp3_flag, ugv_opt_stop1, ugv_opt_stop2, ugv_opt_stop1, ugv_velocity, data_flag, ordered_pts_rest, ordered_pts_from_depot)
            ...
            while flag3:
                ...
                fitness_val3, mission_time3, total_distance3, uav_time, velocity1_3, velocity2_3, route_dict3, route3, tw_dict3, disttravel_track3, ugv_travel_time, ugv_inter_locs = sp3.main(ugv_opt_stop1, fleeting_location, destination_loc, stp2_depart_time, recharge_time, ugv_stops_dict, visited_missions, num_uavs, data_flag)
            ...
            for i in range(len(reversed_ugv_visits2)-1):
                each_stop_ugv_travel_T = ugv_distance_calc_auto.distance_btw_ugvstops(reversed_ugv_visits2[i], reversed_ugv_visits2[i+1], ordered_pts_rest) // ugv_velocity
                timet += each_stop_ugv_travel_T
                ugv_visits_time_track[reversed_ugv_visits2[i]] = timet
            print(reversed_ugv_visits2)
            if len(reversed_ugv_visits2) >= 2:
                ugv_visits_time_track[reversed_ugv_visits2[-1]] = timet + ugv_distance_calc_auto.distance_btw_ugvstops(reversed_ugv_visits2[-2], reversed_ugv_visits2[-1], ordered_pts_rest) // ugv_velocity
            else:
                ugv_visits_time_track[reversed_ugv_visits2[-1]] = timet
            stop1 = True
            stop2 = False
            ...
            if stop1:
                if ugv_max_capacity - ugv_power > ugv_power_direct_from_stop1:
                ...
                if ugv_max_capacity - ugv_power > ugv_power_direct_from_stop2:
    ...
    for key, val in track_node_age.items():
        if len(val) < 3:
            if key in ugv_visits1 or key in ugv_visits2:
                penalty += 0
            else:
                penalty += 1_000_000
    ...
    for key, val in track_node_age.items():
        if len(val) < 3:
            if key in ugv_visits1 or key in ugv_visits2:
                ...
                penalty += 1_000_000
    print(f'The total score accquired during this mission for local optimization is: {mission_end_time+penalty}')
...
if __name__ == "__main__":
    ...
    param_list = [10, 4, 6, 6]  # optimal UGV solution for the reduced scenario - RS = 100
    ...
    num_uavs = 1
    ugv_stops_dict = {}
    mission_points_ordered_df = pd.read_csv('Scenario 11 ordered rest.csv')
    ...
    ugv_Stops = minimum_set_cover_UGV_hyperparam_refined.main(mission_points_ordered_df, 'Case_Study_scenario6.csv')
