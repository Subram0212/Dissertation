...
def create_data_model(strt_pt, sp1_end_point, stp1_ugv_stop_list, stp_depart_time, visited_missions, num_uavs, added_mission_pts, Age_period):
    n_depot_1 = 6
    n_stop_1 = 6
    n_depot_2 = 6
    ugv_velocity = 15
    ...
    depot_b_dist = ugv_distance_calc_auto.distance_from_depotB((ordered_pts_from_depot[0][0]*5280, ordered_pts_from_depot[0][1]*5280), stp1_ugv_stop_ft)
    ...
    uav_strt_time = (ugv_distance_calc_auto.distance_from_depotB_btw_ugvstops(ordered_pts_from_depot[0], strt_pt) // ugv_velocity) + stp_depart_time
    nw_stop_tw_1 = uav_strt_time + int(depot_b_dist // ugv_velocity)
...
def main(strt_pt, sp1_end_point, stp1_ugv_stop, stp_depart_time, visited_missions, num_uavs, added_mission_pts, Age_period):  # This is where optimization of UAV routes happen.
    """Entry point of the program."""
    # Instantiate the data problem.
    # [START data]
    print(f'------------------------------------------------Subproblem 1-------------------------------------------')
    print(f'The corresponding UGV hyperparameters are: Stop1: {stp1_ugv_stop}')
    data, veh_max_time, depotb_vel, ugv_vel, tw_dict, ugv_inter_locs = create_data_model(strt_pt, sp1_end_point, stp1_ugv_stop, stp_depart_time, visited_missions, num_uavs, added_mission_pts, Age_period)
    # [END data]
