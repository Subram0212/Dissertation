...
import overall_PS_multiUGVstops_UGVUAVmoving_scenario4 as PS
...
def global_minimum(X_list, num_uavs, stops_comb_len, normalized_SA, clustering, label):
    ...
    file_path = "Predictor agent classification plot scenario 4/X_kNN_val_10_scn4_"+str(ga_count)+".csv"  # Change this name while varying the classifier
    ...
    file_path_y = "Predictor agent classification plot scenario 4/y_kNN_val_10_scn4_"+str(ga_count)+".csv"   # Change this name while varying the classifier
    np.savetxt(file_path_y, predicted_cluster_list, delimiter=',', fmt='%d')

    with open('output_10_scn4_kNN_'+str(ga_count)+'.txt', 'w') as file:
...
ordered_pts_rest_df = pd.read_csv('Scenario 3 ordered rest.csv')
...
reversed_ordered_pts_rest = list(reversed(ordered_pts_rest))
ugv_stops_dict = {}
ugv_Stops = minimum_set_cover_UGV_hyperparam_refined.main(ordered_pts_rest_df, 'Case_Study_scenario3.csv')
...
# with open('output_100_scn3_GP_0.6_'+str(ga_count)+'.txt', 'w') as file:
#     # Write data for each row
#     for i in range(len(sorted_initial_population)):
#         row = sorted_initial_population[i]
#         other_value = obj_val_list_class[i]
#         row_str = '\t'.join(str(x) for x in row)
#         file.write(f"{row_str}\t{other_value}\n")
