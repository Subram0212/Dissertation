...
def modified_CP(df):
    def cumulative_distance_determination (df):
        ...
        branch1 = [(X[7],Y[7])]+ branch1[::-1]
        branch2 = [(X[7],Y[7])]+ branch2
        branch3 = [(X[7],Y[7])]+ branch3
        ############# branches ####################
        co = branch1
        co1 = branch2
        co2 = branch3
...
def f_matrix(set1, set2):
    radius = 24750/5280
    count = 0
    data = pd.read_excel('ARL corridor points ordered.xlsx')
...
def main(scenario_points, df, scenario_num=5):
    ...
    for i, val in enumerate(solutions):
        for j in range(1, len_rendezvous_pts):
            ugv_stops['Stp_'+str(j)].append(solutions[i][j])
    ...
    for i in range(len(stops_combo_list[1])):
        ...
        target_hit_count = f_matrix(stp_set_1, stp_set_2)  # This function filters out the unnecessary min-max range of two stop location region.
        # If a stop set does not cover all the mission points for all the potential stop location pairs in that region, then eliminate that min-max range.
    ...
    if len(solutions[0]) == 3 and scenario_num == 20:
        stops_combo_list = []
        max_set_count = []
        for key, val in ugv_stops_arranged.items():
            stops_combo_list.append(list(combinations(val, 2)))  # this '2' represents the min-max stop points
...
if __name__ == "__main__":
    mission_pts = pd.read_excel('ARL corridor points ordered.xlsx', engine='openpyxl')
    res = main(mission_pts, 'Case_Study_scenario_remaining_2.csv')
    print(res)
