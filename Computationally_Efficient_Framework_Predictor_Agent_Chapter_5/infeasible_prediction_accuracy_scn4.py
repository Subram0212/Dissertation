...
import minimum_set_cover_UGV_hyperparam_refined_scenario3
import ugv_distance_calc_auto_scenario3
...
with open('output_10_scn4_ensemble.txt', 'r') as file:
    for line in file:
        values = line.split()
        if len(values) >= 5:
...
ugv_stops_dict = {}
ugv_Stops = minimum_set_cover_UGV_hyperparam_refined_scenario3.main(ordered_pts_rest_df, 'Case_Study_scenario3.csv')
...
cax = ax.matshow(conf_matrix, cmap=plt.cm.Oranges, vmin=0, vmax=150)
...
plt.savefig('Confusion matrix and scores/confusion_matrix - ensemble, scenario 4 and seed 10.pdf', format='pdf')
...
plt.savefig('Confusion matrix and scores/ROC curve - ensemble, scenario 4 and seed 10.pdf', format='pdf')
