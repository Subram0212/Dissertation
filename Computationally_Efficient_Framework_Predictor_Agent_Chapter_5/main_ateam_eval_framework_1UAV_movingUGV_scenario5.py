...
import overall_PS_multiUGVstops_UGVUAVmoving_scenario5 as PS
import minimum_set_cover_UGV_hyperparam_refined_scenario5 as minimum_set_cover_UGV_hyperparam_refined
import ugv_distance_calc_auto_scenario5 as ugv_distance_calc_auto
...
def global_minimum(X_list, num_uavs, stops_comb_len, normalized_SA, clustering, label):
    ...
    file_path = "Predictor agent classification plot scenario 5/X_ensemble_val_100_scn5_"+str(ga_count)+".csv"  # Change this name while varying the classifier
    ...
    file_path_y = "Predictor agent classification plot scenario 5/y_ensemble_val_100_scn5_"+str(ga_count)+".csv"   # Change this name while varying the classifier
    np.savetxt(file_path_y, predicted_cluster_list, delimiter=',', fmt='%d')

    with open('output_100_scn5_ensemble_'+str(ga_count)+'.txt', 'w') as file:
...
ordered_pts_rest_df = pd.read_csv('Scenario 11 ordered rest.csv')
...
reversed_ordered_pts_rest = list(reversed(ordered_pts_rest))
ugv_stops_dict = {}
ugv_Stops = minimum_set_cover_UGV_hyperparam_refined.main(ordered_pts_rest_df, 'Case_Study_scenario11.csv')
...
space = Space([(2, stops_comb_len+1), (2, 3), (2, 3), (2, 3)])
space_list = [(2, stops_comb_len+1), (2, 3), (2, 3), (2, 3)]
