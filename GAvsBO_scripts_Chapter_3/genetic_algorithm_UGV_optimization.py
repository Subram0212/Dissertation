...
def operator_selection(init_population_sorted, evolution_with_obj):  # This function takes current generation population obtained from evolution as input arguments
    ...
    for i in range(len(init_population_sorted)//2 + 2):
        ...
        if i < 4:
            ...
            del evolution_with_obj_copy[i][-2]
            next_generation.append(evolution_with_obj_copy[i])
            # evolution_with_obj_copy.pop(0)
...
def mate(father, mother):  # This function performs crossover and mutation and takes two different UGV parameter lists as input arguments.
    ...
    if int(offspring1_list[0]) > 1110011 or int(offspring1_list[0]) < 10:
        if int(offspring1_list[0]) > 1110011:
            offspring1_list[0] = '01110011'
    ...
    if int(offspring2_list[0]) > 1110011 or int(offspring2_list[0]) < 10:
        if int(offspring2_list[0]) > 1110011:
            offspring2_list[0] = '01110011'
...
space = Space([(2, 115), (120//60, 3000//60), (120//60, 3000//60)])
...
for n in range(number_of_generations):
    if n == 0:
        ...
        for j in range(len(sorted_initial_population_copy)):
            ...
            fitness_val, max_time, total_distance, total_time, depotb_vel, ugv_vel, penalty, route_dict, route, stop1, stop2, tw_dict = inner_loop_optimization.main(param_list[0], param_list[1], param_list[2])
            if penalty > 0:
                fitness_val += penalty
            ...
            if j == 0:
                csv_file = open("GA generation dataset/ICUAS '22 energy optimization in GA_gen_"+str(n+1)+"_.csv", "w", newline='')
                writer = csv.writer(csv_file)
                writer.writerow(duration_dict[j+1])
            else:
                csv_file = open("GA generation dataset/ICUAS '22 energy optimization in GA_gen_"+str(n+1)+"_.csv", "a", newline='')
                writer = csv.writer(csv_file)
                writer.writerow(duration_dict[j+1])
