...
def create_data_model(ugv_stop_loc, NW_stop, SE_stop):
    ...
    data["ends"] = [n_sum_rch_stops]*data["num_vehicles"]
...
def print_solution(data, manager, routing, solution):  # Prints the UAV optimization solution output on the console.
    ...
    for vehicle_id in range(data["num_vehicles"]):
        ...
        wall_time_list.append(solution.Value(time_var))
    ...
    wall_time = max(wall_time_list)
    return solution.ObjectiveValue(), total_distance, total_time, wall_time
...
def main(ugv_stop_loc, NW_stop, SE_stop):  # This is where optimization of UAV routes happen.
    ...
    if solution:
        obj_val, total_dist, total_time, wall_time = print_solution(data, manager, routing, solution)
        route_dict, route = get_routes(solution, routing, manager)
        if obj_val < 1_000_000:
            print("Objective value is: {}".format(veh_max_time - wall_time))
            return veh_max_time - wall_time, veh_max_time, total_dist, total_time, depotb_vel, ugv_vel, pen, route_dict, route, stop1, stop2, tw_dict
        else:
