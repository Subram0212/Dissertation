...
def local_minimum(X, improvement_process):
    minimum = minimize(fun=inner_loop_optimization_for_improver.main, x0=X, args=improvement_process, method='Nelder-Mead', callback=cb, options={'maxfev': 10}, tol=0.5)
    ...
    imp_fitness_val, imp_max_time, imp_total_distance, imp_total_time, imp_depotb_vel, imp_ugv_vel, imp_penalty, imp_route_dict, imp_route, imp_stop1, imp_stop2, imp_tw_dict = inner_loop_optimization.main([int(imp_0), int(imp_1), int(imp_2), int(imp_3)])
    ...
    if imp_fitness_val < 6_000_000:
        ...
        imp_1 = wt_time_1
        imp_2 = wt_time_2
        fitness_val, max_time, total_distance, total_time, depotb_vel, ugv_vel, penalty, route_dict, route, stop1, stop2, tw_dict = inner_loop_optimization.main([imp_0, imp_1, imp_2, imp_3])
        if penalty > 0:
            fitness_val += penalty
...
def global_minimum(X_list):
    ...
    for sol in evolution_gen:
        if tuple(sol) in population_dict.keys():
            continue
        global_f, global_t, global_d, global_tt, global_depotvel, global_ugvvel, global_pen, global_rd, global_r, global_stop1, global_stop2, global_tw_dict = inner_loop_optimization.main([sol[0], sol[1], sol[2], sol[3]])
...
n_samples = 30
space = Space([(2, 58), (120//60, 3000//60), (120//60, 3000//60), (2, 5)])  # try having only one depot location - dont forget to add multi depot afterwards
...
# ugv_Stops = [[(3.1, 7.24), (7.45, 3.86)], [(3.1, 7.24), (7.7, 3.61)], [(3.1, 7.24), (8.04, 3.28)], [(3.1, 7.24), (8.39, 2.98)],
#              [(3.1, 7.24), (8.75, 2.73)], [(3.1, 7.24), (9.13, 2.5)], [(3.1, 7.24), (9.53, 2.29)], [(3.1, 7.24), (10.54, 0.9)]]
...
while flag == 1:
    ...
    Thread(target=wrapper1, args=(local_minimum, switch_sol_array, improvement_process, q1)).start()
