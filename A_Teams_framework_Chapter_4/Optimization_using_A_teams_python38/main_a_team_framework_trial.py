...
population_dict = {}
n_samples = 60
space = Space([(2, 68), (120//60, 3000//60), (120//60, 3000//60)])  # try having only one depot location - dont forget to add multi depot afterwards
...
for n in range(len(sorted_initial_population)):
    ...
    #     # if fitness_val == 0 or total_distance == 0 or total_time == 0:
    #     #     fitness_val = 50_000_000
    if fitness_val < 1_000_000 or fitness_val <= 2_000_000:  # Even though initial pop is randomized, this framework allows to perform local min even if the solution is infeasible but close to feasibility.
...
while flag == 0:
    ...
    for key, val in improvement_process.items():
        ...
        if val > 1_000_000:
            continue
        population_dict[(int(key[0]), int(key[1]), int(key[2]))] = int(val)
    ...
    if imp_fitness_val < 1_000_000:
        ...
        if fitness_val == 0 or total_distance == 0 or total_time == 0:
            fitness_val = 50_000_000
        population_dict[(int(imp_0), int(imp_1), int(imp_2))] = fitness_val
    ...
    for sol in evolution_gen:
        ...
        if global_f > 1_000_000:
            continue
        population_dict[tuple(sol)] = global_f
    count = 0
