...
def mate(father, mother):  # This function performs crossover and mutation and takes two different UGV parameter lists as input arguments.
    ...
    print(bin_father_str)
    print(bin_mother_str)
...
def ga_evaluation1(X):
    ...
    for j in range(len(sorted_initial_population_copy)):
        ...
        fitness_val, max_time, total_distance, total_time, depotb_vel, ugv_vel, penalty, route_dict, route, tw_dict, disttravel_track = scenario1_innerloop_opt_2stops.main(param_list[0], param_list[1], param_list[2], param_list[3], ugv_stops_distance_dict)
        if penalty > 0:
            fitness_val += penalty
        if fitness_val == 0 or total_distance == 0 or total_time == 0:
            fitness_val = 50_000_000
        if len(disttravel_track) > 1 and 198*((disttravel_track[-1] - disttravel_track[-2]) // 33) > 287700:
            cnt += 1
            code_switch = 1
            fitness_val, max_time, total_distance, total_time, depotb_vel, ugv_vel, penalty, route_dict, route, tw_dict, \
            disttravel_track = scenario1_innerloop_opt_2stops_updated_solution.main(param_list[0], param_list[1], param_list[2], param_list[3], ugv_stops_distance_dict)
            if fitness_val < 5_000_000:
                ...
                fitness_val, max_time, total_distance, total_time, depotb_vel, ugv_vel, penalty, route_dict, route, tw_dict, disttravel_track = scenario1_innerloop_opt_2stops_updated_solution.main(param_list[0], param_list[1], param_list[2], param_list[3], ugv_stops_distance_dict)
                if penalty > 0:
                    fitness_val += penalty
                if fitness_val == 0 or total_distance == 0 or total_time == 0:
                    fitness_val = 50_000_000
        if fitness_val < 5_000_000 and cnt == 0:
            wt_time_1 = int(param_list[1]) * 60
            wt_time_2 = int(param_list[2]) * 60
            count_stp1 = 0
            count_stp2 = 0
...
print(random_state)
n_samples = 30
number_of_generations = 11
space = Space([(2, 141), (120//60, 3000//60), (120//60, 3000//60), (2, 5)])
