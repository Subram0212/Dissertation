...
import multipleUGV_stops_locs_scenario1
...
def operator_selection(init_population_sorted, evolution_with_obj):  # This function takes current generation population obtained from evolution as input arguments
    ...
    for i in range(len(init_population_sorted)//2 + 2):
        ...
        if i < 4:
            ...
            del evolution_with_obj_copy[i][-3]
...
def mate(father, mother):  # This function performs crossover and mutation and takes two different UGV parameter lists as input arguments.
    ...
    chromosomes_father = bin_father_str[0]+bin_father_str[1]+bin_father_str[2]+bin_father_str[3]+bin_father_str[4]
    chromosomes_mother = bin_mother_str[0]+bin_mother_str[1]+bin_mother_str[2]+bin_mother_str[3]+bin_mother_str[4]
    ...
    stop_val_bin = offspring1_list[4]
    if stop_val_bin == '00000010':
        ...
        if int(offspring1_list[0]) > 11111111 or int(offspring1_list[0]) < 10:
            if int(offspring1_list[0]) > 11111111:
                offspring1_list[0] = '11111111'
            if int(offspring1_list[0]) < 10:
                offspring1_list[0] = '00000010'
    ...
    if int(offspring1_list[3]) > 101 or int(offspring1_list[3]) < 101:
        if int(offspring1_list[3]) > 101:
            offspring1_list[3] = '00000101'
        if int(offspring1_list[3]) < 101:
            offspring1_list[3] = '00000101'
    if int(offspring1_list[4]) > 11 or int(offspring1_list[4]) < 10:
        if int(offspring1_list[4]) > 11:
            offspring1_list[4] = '00000011'
        if int(offspring1_list[4]) < 10:
            offspring1_list[4] = '00000010'
    offspring1 = offspring1_list[0]+offspring1_list[1]+offspring1_list[2]+offspring1_list[3]+offspring1_list[4]
    offspring2_list = bitstring_to_param_list(offspring2)
    stop_val_bin_2 = offspring2_list[4]
    if stop_val_bin_2 == '00000010':
        ...
        if int(offspring2_list[0]) > 11111111 or int(offspring2_list[0]) < 10:
            if int(offspring2_list[0]) > 11111111:
                offspring2_list[0] = '11111111'
            if int(offspring2_list[0]) < 10:
                offspring2_list[0] = '00000010'
    ...
    if int(offspring2_list[3]) > 101 or int(offspring2_list[3]) < 101:
        if int(offspring2_list[3]) > 101:
            offspring2_list[3] = '00000101'
        if int(offspring2_list[3]) < 101:
            offspring2_list[3] = '00000101'
    if int(offspring2_list[4]) > 11 or int(offspring2_list[4]) < 10:
        if int(offspring2_list[4]) > 11:
            offspring2_list[4] = '00000011'
        if int(offspring2_list[4]) < 10:
            offspring2_list[4] = '00000010'
    offspring2 = offspring2_list[0]+offspring2_list[1]+offspring2_list[2]+offspring2_list[3]+offspring2_list[4]
...
space = Space([(120//60, 3000//60), (120//60, 3000//60), (2, 5), (2, 3)])
...
for n in range(number_of_generations):
    if n == 0:
        ...
        for j in range(len(sorted_initial_population_copy)):
            ...
            if param_list[3] == 2:
                space_for_stops = Space([(2, 141)])
                lhs_for_stops = Lhs(lhs_type="classic", criterion=None)
                x_stops = lhs_for_stops.generate(space_for_stops.dimensions, 1)
                param_list.insert(0, x_stops[0][0])
                print("Actual param list is:{}".format(param_list))
            else:
                space_for_stops = Space([(2, 255)])
                lhs_for_stops = Lhs(lhs_type="classic", criterion=None)
                x_stops = lhs_for_stops.generate(space_for_stops.dimensions, 1)
                param_list.insert(0, x_stops[0][0])
                print("Actual param list is:{}".format(param_list))
            num_stops = param_list[4]
            if num_stops == 2:
                duration_dict[j+1] = [ugv_stops_dict[param_list[0]], param_list[1], param_list[2]]
            else:
                if j == 0 or num_stops == 3:
                    ugv_stops_dict_2 = scenario_1_data_processing.scenario_1_data_processing(num_stops)
                duration_dict[j+1] = [ugv_stops_dict_2[param_list[0]], param_list[1], param_list[2]]
            fitness_val, max_time, total_distance, total_time, depotb_vel, ugv_vel, penalty = scenario1_innerloop_opt.main(param_list[0], param_list[1], param_list[2], param_list[3], param_list[4], ugv_stops_distance_dict)
            if penalty > 0:
                fitness_val += penalty
            ...
            duration_dict[j+1].append(param_list[4])
            # duration_dict[j+1].append(alpha[i])
            evolution_with_obj_val.append([param_list[0], param_list[1], param_list[2], fitness_val, param_list[3], param_list[4]])
            if j == 0:
                csv_file = open("Scenario 1 GA dataset/Diff start pt diff_stops randomseed_1_complete selection process in GA_gen_"+str(n+1)+"_.csv", "w", newline='')
                writer = csv.writer(csv_file)
                writer.writerow(duration_dict[j+1])
            else:
                csv_file = open("Scenario 1 GA dataset/Diff start pt diff_stops randomseed_1_complete selection process in GA_gen_"+str(n+1)+"_.csv", "a", newline='')
                writer = csv.writer(csv_file)
                writer.writerow(duration_dict[j+1])
