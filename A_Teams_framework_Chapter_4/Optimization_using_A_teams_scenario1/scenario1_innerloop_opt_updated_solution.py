...
def create_data_model(ugv_stop_loc, NW_stop, SE_stop, strt_pt, ugv_stops_dict, ugv_stops_distance_dict):
    n_depot_1 = 6
    n_stop_1 = 6
    n_stop_2 = 6
    n_depot_2 = 6
    ...
    if strt_pt == 2:
        ...
        print(data["stations"])
        print(data["visits"])
        return data, veh_max_time, mid_pt_to_nw_velocity, velocity, pen
    elif strt_pt == 4:
        print("Start point is: {}".format(strt_pt))
        n_sum_rch_stops = n_depot_1 + n_stop_1 + n_stop_2 + n_depot_2
        ...
        print(data["stations"])
        print(data["visits"])
        return data, veh_max_time, depot_a_to_nw_velocity, velocity, pen
    else:
        print("Start point is: {}".format(strt_pt))
        n_sum_rch_stops = n_depot_1 + n_stop_1 + n_stop_2 + n_depot_2
        ordered_mission_pts_stp1_stop = [(4.09, 0.96), (4.21, 1.44), (4.33, 1.91),
        ...
        if nw_stop_tw_1 > nw_stop_tw_2 or se_stop_tw_1 > se_stop_tw_2:
            print("Warning!!!! Do not consider this solution with parameters {}, {} and {}".format(ugv_stop_loc, NW_stop, SE_stop))
            ugv_time_windows = ([(0, veh_max_time)] +  # 0 (start)
                                [(0, veh_max_time)] * n_depot_1 +  # 1-6
                                [(0, veh_max_time)] * n_stop_1 +  # 7-12
                                [(0, veh_max_time)] * n_stop_2 +  # 13-18
                                [(0, veh_max_time)] * n_depot_2 +  # 19-24
                                [(0, veh_max_time)] * total_mission_pts
                                )
        ...
        if total_ugv_power > 25010000:
            ...
            pen = 0
        print(total_ugv_power)
        print("The locations corresponding to {}, {} and {} are: {} ".format(ugv_stop_loc, NW_stop, SE_stop, _locations))
        ...
        # else:
        #     ugv_stops_dict = scenario_1_data_processing.scenario_1_data_processing(num_stops)
        #     data, veh_max_time, depot_b_to_nw_velocity, velocity, pen = multipleUGV_stops_locs_scenario1.ugv_data(strt_pt, start_loc, num_stops, ugv_stop_loc, ugv_stops_dict, NW_stop, SE_stop, ordered_mission_pts_stp1_stop, ordered_mission_pts_stp2_stop)
        #     return data, veh_max_time, depot_b_to_nw_velocity, velocity, pen


def solution_route_update(data, ugv_stop_loc, strt_pt, ugv_stops_distance_dict, time_windows_dict, uav_time, rechg_at_strt_pt):
    ordered_mission_pts_stp1_stop = [(4.09, 0.96), (4.21, 1.44), (4.33, 1.91),
                                     (4.45, 2.39), (4.57, 2.86), (4.69, 3.39), (4.81, 3.81), (4.92, 4.29), (5.04, 4.76), (5.16, 5.24), (5.28, 5.71),
                                     (5.40, 6.19), (5.52, 6.67), (5.64, 7.14), (5.76, 7.62), (5.88, 8.09)]
    ordered_mission_pts_stp1_stop = [ele for ele in reversed(ordered_mission_pts_stp1_stop)]
    ...
    uav_addnl_time = 0
    ugv_addnl_time = 0
    current_uav_time = uav_time + rechg_at_strt_pt
    ugv_wait_time = 0
    recharge_time = 900
    ugv_speed = 13
    if strt_pt == 4:
        ...
        if ugv_stops_dict[int(ugv_stop_loc)][0] in ordered_mission_pts_stp1_stop:
            ...
            if current_uav_time < time_windows_dict[7][0]:
                ...
                uav_time_taken_from_depot_to_stop += diff
            elif current_uav_time > time_windows_dict[7][1]:
...
def main(params, ugv_stops_dict, ugv_stops_distance_dict, f_eval={}):  # This is where optimization of UAV routes happen.
    """Entry point of the program."""
    # Instantiate the data problem.
    # [START data]
    print("------------------------------------------------------------------------")
    print("****************** Time for a feedback!!!! *************************")
    print("------------------------------------------------------------------------")
    # strt_pt = 5
    ugv_stop_loc, NW_stop, SE_stop, strt_pt = params
    data, veh_max_time, depotb_vel, ugv_vel, pen, tw_dict = create_data_model(ugv_stop_loc, NW_stop, SE_stop, strt_pt, ugv_stops_dict, ugv_stops_distance_dict)
    # [END data]
...
if __name__ == '__main__':
    params = [105, 25, 34, 5]
    ugv_stops_dict = scenario_1_data_processing.scenario_1_data_processing()
    ugv_stops_dist_dict = ugv_distance_calc_2()
    graph = main(params, ugv_stops_dict, ugv_stops_dist_dict)
