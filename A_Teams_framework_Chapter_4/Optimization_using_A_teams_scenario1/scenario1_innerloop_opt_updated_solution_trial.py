...
def create_data_model(ugv_stop_loc, NW_stop, SE_stop, strt_pt, ugv_stops_dict, ugv_stops_distance_dict, add_ugv_stop):
    n_depot_1 = 6
    n_stop_1 = 6
    n_stop_2 = 6
    n_depot_2 = 6
    ...
    if strt_pt == 2:
        ...
        assert data['num_vehicles'] == len(data['starts'])
        assert len(data["counter"]) == len(data['time_windows'])
        data["stations"] = [i for i in range(0, 21)]
        data["visits"] = [i for i in range(22, len(_locations)) if i != 24]
        print(data["stations"])
        print(data["visits"])
        ...
        _locations = ([(8.91, 4.54)] +
                      [(8.91, 4.54)] * n_depot_1 +  # 6
                      stp1_ugv_stop_list * n_stop_1 +  # 12
                      stp2_ugv_stop_list * n_stop_2 +  # 18
                      [(0.0, 0.0)] * (n_depot_2 - 2) +  # 22
                      [add_ugv_stop] * (n_depot_2 - 5) +  # 23
                      [(8.91, 4.54)] # 24
                      )
        mission_locations = []
        count = 0
        ...
        if stp1:
            ugv_time_windows = ([(0, 60)] +  # 0 (start)
                                ...
                                [(nw_stop_tw_1, nw_stop_tw_2)] * n_stop_1 +  # 7-12
                                [(se_stop_tw_1, se_stop_tw_2)] * n_stop_2 +  # 13-18
                                [(0, veh_max_time)] * (n_depot_2 - 2) +  # 19-22
                                [(nw_stop_tw_2, se_stop_tw_1)] +  #23 - Extra UGV stop
                                [(nw_stop_tw_2, veh_max_time)] +  #24
                                [(0, veh_max_time)] * total_mission_pts
                                )
        elif stp2:
            ugv_time_windows = ([(0, 60)] +  # 0 (start)
                                ...
                                [(se_stop_tw_1, se_stop_tw_2)] * n_stop_2 +  # 13-18
                                [(0, veh_max_time)] * (n_depot_2 - 2) +  # 19-22
                                [(se_stop_tw_1, veh_max_time)] +  #23 - Extra UGV stop
                                [(se_stop_tw_2, veh_max_time)] +  #24
                                [(0, veh_max_time)] * total_mission_pts
                                )
        ...
        assert len(data["counter"]) == len(data['time_windows'])
        data["stations"] = [i for i in range(0, n_sum_rch_stops)]
        data["visits"] = data["visits"] = [i for i in range(n_sum_rch_stops+1, len(_locations))]
        print(data["stations"])
        print(data["visits"])
        time_windows_dict = {}
        for i, time_ in enumerate(data["time_windows"]):
            time_windows_dict[i] = time_
        return data, veh_max_time, depot_c_to_nw_velocity, velocity, pen, time_windows_dict, ordered_mission_pts_stp1_stop, ordered_mission_pts_stp2_stop
...
def main(params, ugv_stops_dict, ugv_stops_distance_dict, add_ugv_stop, f_eval={}):  # This is where optimization of UAV routes happen.
    """Entry point of the program."""
    # Instantiate the data problem.
    # [START data]
    strt_pt = 5
    ugv_stop_loc, NW_stop, SE_stop = params
    data, veh_max_time, depotb_vel, ugv_vel, pen, tw_dict, ordered_mission_stp1s, ordered_mission_stp2s = create_data_model(ugv_stop_loc, NW_stop, SE_stop, strt_pt, ugv_stops_dict, ugv_stops_distance_dict, add_ugv_stop)
    # [END data]
    ...
    if solution:
        ...
        return obj_val, veh_max_time, total_dist, total_time, depotb_vel, ugv_vel, pen, route_dict, route, tw_dict, ordered_mission_stp1s, ordered_mission_stp2s
    else:
