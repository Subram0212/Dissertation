...
df = pd.read_excel('plotting scenario2 another solution diff start point.xlsx')
# print(df)
...
new_list = [[i, 0, 0] for i in range(161)]
# print(new_list)
...
ugv_list = [[0, 0] for j in range(161)]
for i in range(len(ugv_list)):
    if i == 0:
        ugv_list[i] = [0, 0]
    if i == 54:
        ugv_list[i] = [5.35, 5.95]
    if 58 <= i <= 79:
...
df5.to_excel('data_to_interpolate_scenario2_midpt start.xlsx', index=False, engine='openpyxl')
