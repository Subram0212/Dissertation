...
def create_data_model(ugv_stop_loc, NW_stop, SE_stop, strt_pt, ugv_stops_dict, ugv_stops_distance_dict):
    ...
    if int(ugv_stop_loc) < 2 or int(NW_stop) < 2 or int(SE_stop) < 2 or strt_pt < 2:
        if int(ugv_stop_loc) < 2:
            ugv_stop_loc = 2
        if int(NW_stop) < 2:
            NW_stop = 2
        if int(SE_stop) < 2:
    ...
    if strt_pt == 2:
        ...
        veh_max_time = se_stop_tw_2 + (return_dist_from_se//velocity) + (return_dist_to_depotB//mid_pt_to_nw_velocity)
...
def print_solution(data, manager, routing, solution):  # Prints the UAV optimization solution output on the console.
    ...
    # df2.to_excel('plotting scenario2 another solution diff start point.xlsx', index=False, engine='openpyxl')
    # if j == 7 and alpha == 1:
    #     df2 = pd.DataFrame(coord_list, columns=['x', 'y'])
    #     # df2.insert(0, 'time', time_elapsed)
    #     # df2.insert(3, 'fuel remaining', fuel_remaining)
