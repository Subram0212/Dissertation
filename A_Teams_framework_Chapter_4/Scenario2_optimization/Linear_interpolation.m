...
while m <= (length(x_prime)-recharge_time)
    if ((x_prime(m)==0 && y_prime(m)==0) && m ~= 1)
        for n=m:m+recharge_time
            x_prime(n) = 0;
            y_prime(n) = 0;
        end
        m = m + recharge_time;
    elseif ((x_prime(m)==5.77 && y_prime(m)==5.83) && m ~= 1)
        for n=m:m+recharge_time
            x_prime(n) = 5.77;
            y_prime(n) = 5.83;
        end
        m = m + 14;
    elseif ((x_prime(m)==5.35 && y_prime(m)==6.1) && m ~= 1)
        for n=m:m+recharge_time
            x_prime(n) = 5.35;
            y_prime(n) = 6.1;
        end
...
%         end
%         m = m + recharge_time;
%     elseif ((x_prime(m)==8.66 && y_prime(m)==5) && m ~= 1)
%         for n=m:m+14
%             x_prime(n) = 8.66;
%             y_prime(n) = 5;
%         end
...
writetable(data_to_animate, 'Int_interpolated_blah.xlsx');
% elseif (ismember(l, u))
%         index = find(u==l);
%         int_value_x1 = interpolate_x1(index+1);
%         int_value_y1 = interpolate_y1(index+1);
