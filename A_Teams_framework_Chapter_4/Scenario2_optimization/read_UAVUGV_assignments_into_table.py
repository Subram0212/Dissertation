...
df = pd.read_excel('plotting GA-LS hierarchical optimized parameter data_scn1.xlsx')
# print(df)
...
new_list = [[i, 0, 0] for i in range(176)]
# print(new_list)
...
ugv_list = [[0, 0] for j in range(176)]
for i in range(len(ugv_list)):
    if i == 0:
        ugv_list[i] = [8.91, 4.54]
    if 11 <= i <= 36:
        ugv_list[i] = [7.68, 3.53]
    elif 95 <= i <= 117:
        ugv_list[i] = [4.92, 4.29]
    # elif 252 <= i <= 284:
...
df5.to_excel('data_to_interpolate_GA_LS_algorithm_scenario1_depotA start.xlsx', index=False, engine='openpyxl')
