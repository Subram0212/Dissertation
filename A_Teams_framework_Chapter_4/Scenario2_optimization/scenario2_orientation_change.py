...
modified_list = [[0, 0] for _ in range(0, len(numpy_array))]
df2 = pd.DataFrame(modified_list, dtype=float)
print(transformation_matrix)
for i in range(len(numpy_array)):
    res = np.matmul(transformation_matrix, np.transpose(numpy_array[i]))
    # df2[i][1] = np.matmul(transformation_matrix, numpy_array[i][1])
    res.astype(int)
    df2[0][i], df2[1][i] = res
    # df2[i][1] = res[1]
print(df2)
df2.to_csv("Scenario 2 orientation change data points.csv", index=False)
...
for i in range(len(df)):
    x.append(df2[0][i]*5280)
    y.append(df2[1][i]*5280)
...
#     ugv_dist = ugv_distance_calc((6.75*5280, 4.51*5280), (9.66*5280, 2.59*5280))
#     print(ugv_dist)
