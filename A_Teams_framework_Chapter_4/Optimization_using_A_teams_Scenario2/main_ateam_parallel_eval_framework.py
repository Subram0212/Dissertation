import numpy as np
import pandas as pd
import scenario2_genetic_algorithm as ga
import scenario2_innerloop_opt_for_improver_agent
...
import scenario2_innerloop_opt_for_improver_agent_updated_solution
...
def unique_evolution(X_list):
    ...
    for i in range(len(X_list)):
        if len(unique_list_stops) == 0:
            break
        if unique_list_stops[0][0] == X_list[i][0] and unique_list_stops[0][1] == X_list[i][3]:
            unique_list.append(X_list[i])
            unique_list_stops.pop(0)
    return unique_list


# X_list = [[34, 23, 8, 5], [52, 26, 21, 3], [32, 23, 8, 4], [54, 26, 21, 3], [52, 23, 8, 5], [34, 26, 21, 5], [3, 20, 381, 4], [124, 11, 10, 5]]
...
def local_minimum(X, code_switch, improvement_process):
    cnt = 0
    if code_switch == 0:
        minimum = minimize(fun=scenario2_innerloop_opt_for_improver_agent.main, x0=X, args=(ugv_stops_dict, ugv_stops_distance_dict, improvement_process), method='Nelder-Mead', bounds=[(2, 9), (2, 50), (2, 50), (2,5)], callback=cb, options={'maxfev': 10}, tol=0.2)
    else:
        minimum = minimize(fun=scenario2_innerloop_opt_for_improver_agent_updated_solution.main, x0=X, args=(ugv_stops_dict, ugv_stops_distance_dict, improvement_process), method='Nelder-Mead', bounds=[(2, 9), (2, 50), (2, 50), (2,5)], callback=cb, options={'maxfev': 10}, tol=0.2)
    ...
    imp_fitness_val, imp_max_time, imp_total_distance, imp_total_time, imp_depotb_vel, imp_ugv_vel, imp_penalty, imp_route_dict, imp_route, imp_tw_dict, disttravel_track = scenario2_innerloop_opt.main([int(imp_0), int(imp_1), int(imp_2), int(imp_3)], ugv_stops_dict, ugv_stops_distance_dict)
    while 198*((disttravel_track[-1] - disttravel_track[-2]) // 33) > 287700 or imp_fitness_val > 6_000_000 or imp_fitness_val == 0:
        if cnt > 3:
            break
        ...
        proper_tw_dict, disttravel_track = scenario2_innerloop_opt_updated_solution.main([int(imp_0), int(imp_1), int(imp_2), int(imp_3)], ugv_stops_dict, ugv_stops_distance_dict)
    ...
    # population_dict[tuple([int(imp_0), int(imp_1), int(imp_2)])] = imp_fitness_val
        if proper_fitness_val < 6_000_000:
            ...
            imp_1 = wt_time_1
            imp_2 = wt_time_2
            fitness_val, max_time, total_distance, total_time, depotb_vel, ugv_vel, penalty, route_dict, route, tw_dict, disttravel_track = scenario2_innerloop_opt_updated_solution.main([imp_0, imp_1, imp_2, imp_3], ugv_stops_dict, ugv_stops_distance_dict)
            if penalty > 0:
                fitness_val += penalty
    ...
    if imp_fitness_val < 6_000_000 and cnt == 0:
        wt_time_1 = int(imp_1) * 60
        wt_time_2 = int(imp_2) * 60
        count_stp1 = 0
        count_stp2 = 0
        ...
        imp_1 = wt_time_1
        imp_2 = wt_time_2
        fitness_val, max_time, total_distance, total_time, depotb_vel, ugv_vel, penalty, route_dict, route, tw_dict, disttravel_track = scenario2_innerloop_opt.main([imp_0, imp_1, imp_2, imp_3], ugv_stops_dict, ugv_stops_distance_dict)
        if penalty > 0:
            fitness_val += penalty
        if fitness_val == 0 or total_distance == 0 or total_time == 0:
            fitness_val = 50_000_000
...
def global_minimum(X_list):
    ...
    for sol in evolution_gen:
        if tuple(sol) in population_dict.keys():
            continue
        code_switch = 0
        global_f, global_t, global_d, global_tt, global_depotvel, global_ugvvel, global_pen, global_rd, global_r, global_tw_dict, disttravel_track = scenario2_innerloop_opt.main([sol[0], sol[1], sol[2], sol[3]], ugv_stops_dict, ugv_stops_distance_dict)
        if global_f == 0:
            global_f = 50_000_000
        while 198*((disttravel_track[-1] - disttravel_track[-2]) // 33) > 287700:
            code_switch = 1
            print("*************************Execute updated global optimization*************************")
            global_f, global_t, global_d, global_tt, global_depotvel, global_ugvvel, global_pen, global_rd, global_r, global_tw_dict, disttravel_track = scenario2_innerloop_opt_updated_solution.main([sol[0], sol[1], sol[2], sol[3]], ugv_stops_dict, ugv_stops_distance_dict)
...
def ugv_distance_calc():
    ...
    for j in range(len(combo)):
        ...
        ugv_stops_distance_dict[combo[j]] = scenario_2_data_processing.ugv_distance(combo[j][0], combo[j][1])
...
n_samples = 30
space = Space([(2, 9), (120//60, 3000//60), (120//60, 3000//60), (2, 5)])  # try having only one depot location - dont forget to add multi depot afterwards
...
ugv_stops_dict = scenario_2_data_processing.scenario_1_data_processing()
...
for n in range(len(sorted_initial_population)):
    ...
    fitness_val, max_time, total_distance, total_time, depotb_vel, ugv_vel, penalty, route_dict, route, tw_dict, disttravel_track = scenario2_innerloop_opt.main(param_list, ugv_stops_dict, ugv_stops_distance_dict)
    if penalty > 0:
        fitness_val += penalty
    if fitness_val == 0 or total_distance == 0 or total_time == 0:
        fitness_val = 50_000_000
    while 198*((disttravel_track[-1] - disttravel_track[-2]) // 33) > 287700:
        code_switch = 1
        proper_fitness_val, proper_max_time, proper_total_distance, proper_total_time, proper_depotb_vel, proper_ugv_vel, proper_penalty, proper_route_dict, proper_route, \
        proper_tw_dict, disttravel_track = scenario2_innerloop_opt_updated_solution.main(param_list, ugv_stops_dict, ugv_stops_distance_dict)
    # if fitness_val == 50_000_000 or proper_fitness_val == 50_000_000 or proper_fitness_val == 0:
    #     continue
...
while flag == 1:
    ...
    print("Population is: {}".format(population_dict))
    itr = 0
    for key, val in population_dict.items():
        if itr == 0:
